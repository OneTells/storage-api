...
async def fetch_transfers(
    ...
    rows = await repositories.fetch_transfers(connection, page, limit, user_id, created_from, created_to)
    total = await repositories.count_transfers(connection, user_id, created_from, created_to)

    return TransfersListResponse(
        items=[TransferRead.model_validate(dict(r)) for r in rows],
...
async def get_transfer(
    ...
    return TransferRead.model_validate(dict(row))
