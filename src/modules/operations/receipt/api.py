...
async def fetch_receipts(
    ...
    rows = await repositories.fetch_receipts(connection, page, limit, user_id, created_from, created_to)
    total = await repositories.count_receipts(connection, user_id, created_from, created_to)

    return ReceiptsListResponse(
        items=[ReceiptRead.model_validate(dict(x)) for x in rows],
...
async def get_receipt(
    ...
    return ReceiptRead.model_validate(dict(row))
