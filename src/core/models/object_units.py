...
from sqlalchemy import BigInteger, ForeignKey, Index, UUID, text, Enum
...
class ObjectUnit(Base):
    ...
    id: MappedColumn[uuid.UUID] = mapped_column(UUID, primary_key=True, server_default=text("uuid_generate_v4()"))
    ...
    status: MappedColumn[ObjectUnitStatus] = mapped_column(Enum(ObjectUnitStatus), nullable=False)
